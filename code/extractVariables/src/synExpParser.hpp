...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YYASSERT (tok == token::T_EOF || tok == token::T_TRUE || tok == token::T_FALSE || tok == token::T_MODULE || tok == token::T_IF || tok == token::T_THEN || tok == token::T_ELSE || tok == token::T_UNEQUAL || tok == token::T_CLOSEPAREN || tok == token::T_OPENPAREN || tok == token::T_EOL || tok == token::T_XOR || tok == token::T_IMPLIES || tok == token::T_OR || tok == token::T_AND || tok == token::T_EQUAL || tok == token::T_NOT);
      }
#else
      ...
      {
        YYASSERT (tok == token::T_MAINMENU || tok == token::T_MENU);
      }
#else
      ...
      symbol_type (int tok, std::string v, location_type l)
        : super_type(token_type (tok), std::move (v), std::move (l))
      {
        YYASSERT (tok == token::T_LITERAL || tok == token::T_SYMBOL);
      }
#else
...
#if 201103L <= YY_CPLUSPLUS
  template <typename Base>
  synExpParser::basic_symbol<Base>::basic_symbol (basic_symbol&& that)
    : Base (std::move (that))
    , value ()
...
#if 201103L <= YY_CPLUSPLUS
  inline
  synExpParser::by_type::by_type (by_type&& that)
    : type (that.type)
  {
    that.clear ();
  }
  ...
  inline
  synExpParser::by_type::by_type (const by_type& that)
    : type (that.type)
  {}
  ...
  inline
  int
  synExpParser::by_type::type_get () const YY_NOEXCEPT
  {
    return type;
  }
  ...
  synExpParser::by_type::token () const YY_NOEXCEPT
  {
...
#line 1587 "src/synExpParser.hpp"
