    ...
    for(std::pair<int, int> lcomp : pcomp->listComponents()) {
        if (lcomp.second > 1) {
            ...
            //if (headNodes != 0 && headNodes != 1) {
                ...
                std::cerr << "Component start with " << headNodes << " nodes in component with start "
            << std::setw(4) << lcomp.first << " length " << std::setw(4) << lcomp.second << " and " << std::setw(5) << showHuman(tnodes)
                << " nodes " << std::endl;
                //std::cerr << "Component members: ";
                for(int x = lcomp.first; x < lcomp.first+lcomp.second; x++) {
                    int  pos = mgr.ReadPerm(mgr.ReadInvPerm(x));
                    std::cerr << std::setw(4) <<  inVars[mgr.ReadInvPerm(x)] << "(" << std::setw(4) << mgr.ReadInvPerm(x) << ") pos "
                             << pos << " ";
                }
                std::cerr << std::endl;
                for(int i = lcomp.first; i < lcomp.first+lcomp.second; i++)
                    if (levelNodes[i] > 0)
                        std::cerr << std::setw(4) << i << " " << std::setw(40) << inVars[mgr.ReadInvPerm(i)] << "("
...
void cuddAdapter::reorder(std::string reorderMethod) {
    ...
    //std::cout << "Reordering. There are " << ts.size() << " components that need reordering" << std::endl;
    //checkComponents();
