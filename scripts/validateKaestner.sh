...
for i in `seq $2`; do
    ...
    if ! BR2_EXTERNAL=support/dummy-external $KCONF/randomconf $FILE >logfile 2>&1; then
        cat logfile
        echo randomconf execution failed
        exit
    fi
    cat configuration $KCONF/systems/$1.exp >$1.$i.exp
    extractVariables $1.$i.exp >$1.$i.var
    if ! $KCONF/bin/Logic2BDD $EXTRA -min-nodes 10000 -base $1.$i  -cudd -constraint-reorder  minspan  $1.$i.var $1.$i.exp  >>logfile 2>&1; then
       cat logfile
       exit
    fi
