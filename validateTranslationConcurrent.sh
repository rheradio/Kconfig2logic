...
function generateAndTest() {
    ...
    if ! ../../code/bin/randomconf $1 $3 >logfile.$1 2>&1; then
        cat logfile.$1
        echo randomconf execution $1 failed
        kill $$
        exit
    fi
    cat configuration.$1 ../../translations/Kconfig2Logic/$2.exp >$2.$1.exp
    if ! ../../code/bin/Logic2BDD $EXTRA -base $2.$1 -min-nodes 1000000  -cudd -constraint-reorder minspan    ../../translations/Kconfig2Logic/$2.var $2.$1.exp  >>logfile.$1 2>&1; then
      cat logfile.$1
      kill $$
      exit
    fi

  solutions=`../../code/bin/counter $2.$1`
